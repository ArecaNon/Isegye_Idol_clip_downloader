...
class CLIP:
    ...
    var  = {0: COMPLETED_PATH,1: Select_member,2: user_id,3: user_pw} #  Select_member / COMPLETED_PATH / user_id / user_pw
            ...
            while(True):   
                ...
                for i in range(pagenum):
                    ...
                    for data in datas:
                        ...
                        """
                        try:
                            article_head = datas[len_-j].find(class_='head').text
                            article_head = re.split(r'\s+', article_head)[1]
                            article_movie = datas[len_-j].find(class_='list-i-movie')
                            if(article_head not in self.heads or article_movie is None):
                                j += 1
                                continue
                        except:
                            j += 1
